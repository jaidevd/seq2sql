...
def load_dataset(dataset_id, use_small=False):
    if dataset_id == 0:
        ...
        sql_data, table_data = load_data(
            "data/train_tok.jsonl", "data/train_tok.tables.jsonl", use_small=use_small
        )
        val_sql_data, val_table_data = load_data(
            "data/dev_tok.jsonl", "data/dev_tok.tables.jsonl", use_small=use_small
        )
        test_sql_data, test_table_data = load_data(
            "data/test_tok.jsonl", "data/test_tok.tables.jsonl", use_small=use_small
        )
        ...
        sql_data, table_data = load_data(
            "data_resplit/train.jsonl", "data_resplit/tables.jsonl", use_small=use_small
        )
        val_sql_data, val_table_data = load_data(
            "data_resplit/dev.jsonl", "data_resplit/tables.jsonl", use_small=use_small
        )
        test_sql_data, test_table_data = load_data(
            "data_resplit/test.jsonl", "data_resplit/tables.jsonl", use_small=use_small
        )
...
def best_model_name(args, for_load=False):
    ...
    agg_model_name = "saved_model/%s_%s%s%s.agg_model" % (
        new_data,
        mode,
        use_emb,
        use_ca,
    )
    sel_model_name = "saved_model/%s_%s%s%s.sel_model" % (
        new_data,
        mode,
        use_emb,
        use_ca,
    )
    cond_model_name = "saved_model/%s_%s%s%s.cond_%smodel" % (
        new_data,
        mode,
        use_emb,
        use_ca,
    ...
    if not for_load and args.train_emb:
        agg_embed_name = "saved_model/%s_%s%s%s.agg_embed" % (
            new_data,
            mode,
            use_emb,
            use_ca,
        )
        sel_embed_name = "saved_model/%s_%s%s%s.sel_embed" % (
            new_data,
            mode,
            use_emb,
            use_ca,
        )
        cond_embed_name = "saved_model/%s_%s%s%s.cond_embed" % (
            new_data,
            mode,
            use_emb,
            use_ca,
...
def epoch_train(model, optimizer, batch_size, sql_data, table_data, pred_entry):
    ...
    while st < len(sql_data):
        ...
        q_seq, col_seq, col_num, ans_seq, query_seq, gt_cond_seq = to_batch_seq(
...
def epoch_exec_acc(model, batch_size, sql_data, table_data, db_path):
    ...
    while st < len(sql_data):
        ...
        gt_where_seq = model.generate_gt_where_seq(q_seq, col_seq, query_seq)  # NOQA: F841
