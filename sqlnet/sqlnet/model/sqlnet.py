...
class SQLNet(nn.Module):
    def __init__(
        ...
        # Word embedding
        if trainable_emb:
            self.agg_embed_layer = WordEmbedding(
                word_emb,
                N_word,
                gpu,
                self.SQL_TOK,
            ...
            self.sel_embed_layer = WordEmbedding(
                word_emb,
                N_word,
                gpu,
                self.SQL_TOK,
            ...
            self.cond_embed_layer = WordEmbedding(
                word_emb,
                N_word,
                gpu,
                self.SQL_TOK,
        ...
        self.cond_pred = SQLNetCondPredictor(
            N_word, N_h, N_depth, self.max_col_num, self.max_tok_num, use_ca, gpu
        )
    ...
    def generate_gt_where_seq(self, q, col, query):
        ...
        for cur_q, cur_col, cur_query in zip(q, col, query):
            cur_values = []
            st = (
                cur_query.index("WHERE") + 1 if "WHERE" in cur_query else len(cur_query)
            )
            all_toks = ["<BEG>"] + cur_q + ["<END>"]
            while st < len(cur_query):
                ed = (
                    len(cur_query)
                    if "AND" not in cur_query[st:]
    ...
    def loss(self, score, truth_num, pred_entry, gt_where):
        ...
        if pred_cond:
            ...
            bce_loss = -torch.mean(
            3 * (cond_col_truth_var * torch.log(cond_col_prob + 1e-10)) +  # NOQA: W504
    ...
    def check_acc(self, vis_info, pred_queries, gt_queries, pred_entry):
        ...
        def gen_cond_str(conds, header):
            ...
            cond_str = []
            for cond in conds:
                cond_str.append(" ".join(
                    header[cond[0]], self.COND_OPS[cond[1]], str(cond[2]).lower())
                )
            return "WHERE " + " AND ".join(cond_str)

        pred_agg, pred_sel, pred_cond = pred_entry

        B = len(gt_queries)  # NOQA: F841

        tot_err = agg_err = sel_err = cond_err = 0.0
        cond_num_err = cond_col_err = cond_op_err = cond_val_err = 0.0
        agg_ops = ["None", "MAX", "MIN", "COUNT", "SUM", "AVG"]  # NOQA: F841
        for b, (pred_qry, gt_qry) in enumerate(zip(pred_queries, gt_queries)):
            ...
            if pred_cond:
                ...
                for idx in range(len(cond_pred)):
                    ...
                    gt_idx = tuple(x[0] for x in cond_gt).index(cond_pred[idx][0])
                    if flag:
                        if str(cond_gt[gt_idx][2]).lower() != str(cond_pred[idx][2]).lower():
                            flag = False
                            cond_val_err += 1
    ...
    def gen_query(
        self, score, q, col, raw_q, raw_col, pred_entry, reinforce=False, verbose=False
    ):
