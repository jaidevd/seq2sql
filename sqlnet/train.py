...
if __name__ == "__main__":
    ...
    parser.add_argument("--ca", action="store_true", help="Use conditional attention.")
    parser.add_argument(
        "--dataset",
        type=int,
        default=0,
    ...
    if args.rl:
        ...
        print(
            "Init dev acc_ex: %s"
            % epoch_exec_acc(model, BATCH_SIZE, val_sql_data, val_table_data, DEV_DB)
        )
        ...
        for i in range(100):
            ...
            print(
                " Loss = %s"
                % epoch_train(
                    model, optimizer, BATCH_SIZE, sql_data, table_data, TRAIN_ENTRY
                )
            )
            print(
                " Train acc_qm: %s\n   breakdown result: %s"
                % epoch_acc(model, BATCH_SIZE, sql_data, table_data, TRAIN_ENTRY)
            )
            # val_acc = epoch_token_acc(
            #     model, BATCH_SIZE, val_sql_data, val_table_data, TRAIN_ENTRY)
            val_acc = epoch_acc(
                model, BATCH_SIZE, val_sql_data, val_table_data, TRAIN_ENTRY
            )
            ...
            if TRAIN_AGG:
                if val_acc[1][0] > best_agg_acc:
                    ...
                    if args.train_emb:
                        torch.save(
                            model.agg_embed_layer.state_dict(),
            ...
            if TRAIN_COND:
                if val_acc[1][2] > best_cond_acc:
                    ...
                    if args.train_emb:
                        ...
                        torch.save(model.cond_embed_layer.state_dict(), cond_e)
            print(
                " Best val acc = %s, on epoch %s individually"
                % (
